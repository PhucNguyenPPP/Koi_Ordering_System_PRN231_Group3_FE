...
import { Button, CircularProgress, FormControl, FormControlLabel, Input, InputLabel, Modal, Pagination, Radio, RadioGroup, TextField, debounce } from '@mui/material';
...
const ShipperManagement = () => {
    ...
    const fetchAllShipperOfStorage = async () => {
        ...
        const response = await GetAllShipperOfStorage(user.storageProvinceId, searchQuery, currentPage, rowsPerPage);
        if (response.ok) {
            ...
            <div className={styles.koiContentContainer}>
                ...
                <TableContainer component={Paper}>
                    <Table sx={{ minWidth: 700 }} aria-label="customized table">
                        <TableHead>
                            <TableRow>
                                <StyledTableCell style={{ fontWeight: 'bold', fontSize: '20px' }}>Shippper Name</StyledTableCell>
                                <StyledTableCell style={{ fontWeight: 'bold', fontSize: '20px' }}>Phone</StyledTableCell>
                                <StyledTableCell style={{ fontWeight: 'bold', fontSize: '20px' }}>Email</StyledTableCell>
                                <StyledTableCell style={{ fontWeight: 'bold', fontSize: '20px' }}>Address</StyledTableCell>
                                <StyledTableCell style={{ fontWeight: 'bold', fontSize: '20px' }}>Gender</StyledTableCell>
                                <StyledTableCell style={{ fontWeight: 'bold', fontSize: '20px' }} align="right">Action</StyledTableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {shipperList.length > 0 ? shipperList.map((shipper) => (
                                <StyledTableRow key={shipper.UserId}>
                                    ...
                                    <StyledTableCell align="right">
                                        <IconButton onClick={(event) => handleMenuClick(event, shipper)}>
                                            <MoreHorizontalIcon />
                                        </IconButton>
                                    </StyledTableCell>
                                </StyledTableRow>
                    ...
                    <div className="bg-white p-5 rounded shadow-md w-1/2">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className="flex justify-end mt-4">
                                <Button type="button" onClick={handleCloseModalCreateShipper} color="primary" className="mr-2">
                                    Cancel
                                </Button>
                                <Button
                                    type="submit"
                                ...
                                >
                                    {isLoading ? (
                                        <>
                                            ...
                                            <span style={{ visibility: 'hidden' }}>CREATE SHIPPER</span> {/* Hide the text while loading */}
                                        </>
                                    ) : (
                                        'CREATE SHIPPER'
                                    )}
